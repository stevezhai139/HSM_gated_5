%% Supplementary Material (Version 5): HSM — Proofs of 4 Lemmas + 6 Theorems
%% Paper: "HSM: Workload-Similarity Gating for Index-Maintenance Decisions,
%%         with Formal Bounds"
%% Author: Arun Reungsilpkolkarn
%% Matches Version 5 of the main paper (6 Theorems + 4 Lemmas,
%% HSM_gated experimental artifact).
...
%% Reduce excessive vertical stretching; let pages end naturally.
\raggedbottom

%% Relax float parameters so floats pack more aggressively
%% (reduces whitespace left when LaTeX defers a figure/table).
...
{\normalsize\bfseries HSM: Workload-Similarity Gating for
...
\texttt{arun.r@bu.ac.th} \quad ORCID: \texttt{0009-0009-2077-0006}}\\[4pt]
\end{center}
\begin{quote}\small
...
\begin{remark}[Deployed $\alpha$ vs.\ worst-case $\alpha^{*}$]
The bound $\alpha^{*}=2k=8$ is the \emph{SAX-only} worst case, i.e.,
the minimum alphabet required when a single SAX encoder must
discriminate all $2k$ regions---the $k$ canonical states \emph{and}
the burst direction (onset vs.\ recovery)---on the raw standardised
QPS axis. The deployed pipeline is not SAX-only: DWT(db4, $L\!=\!3$)
first decomposes each window into approximation coefficients $cA_3$
and detail coefficients $cD_1, cD_2, cD_3$. The detail bands already
encode burst direction through the sign of their wavelet coefficients
at gradient reversals, so SAX operating on the $cA_3$ band need only
discriminate the $k=4$ canonical states (volume regime $\times$
query-type dominance). Under this decomposition of responsibility,
$\alpha=k=4$ is sufficient and is the setting used throughout
Section~V of the main paper, which reports Youden $J\geq0.96$ on
every workload. The bound $\alpha^{*}=2k=8$ is retained as the
conservative fallback for pipelines that omit DWT pre-separation.
\end{remark}
\hfill$\square$
\end{proof}
...
\begin{remark}[Plug-in check]
The algebraic steps $(i)$--$(iv)$ above reduce Theorem~5 to the
elementary rule $\mathrm{Speedup}_\infty=1/(1{-}\pi_w\hat{\TPR}
{-}(1{-}\pi_w)(1{-}\hat{\TNR}))$ once a confidence band for
$(\TPR,\TNR)$ is supplied by Theorem~4.  The worked numerical
application at SF\,=\,3.0 --- including the full
$\pi_w$-parameterised curve and the 95\,\% deployment
certificate --- is stated once in \S X (``Phase-Mix-Parameterised
Speedup at SF\,=\,3.0'').
\end{remark}
...
(i)~measurement cost $a\Npts\log\Npts$, a conservative upper bound
accommodating kernel variants beyond the default FastDTW pipeline
(Lemma~3 establishes the tight $\Theta(\Npts)$ special case);
...
\end{equation}
which is solved by fixed-point iteration from $N_{\mathrm{pts},0}=64$ in
...
fixed. Substituting into \eqref{eq:supp-nstar-implicit} gives
...
\subsection{Cost-Model Parameters}\label{supp:costmodel}

\begin{table}[!htb]\centering\small
\caption{Cost model parameters used in Theorems~3,~5, and~6 of the main paper.}\label{tab:costmodel-supp}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lp{3.1cm}l@{}}
\toprule
\textbf{Symbol} & \textbf{Definition} & \textbf{Complexity}\\
\midrule
$C_{\mathrm{comp}}$   & HSM computation cost     & $c\Npts\!=\!\Theta(\Npts)$\\
$C_{\mathrm{create}}$ & Index creation cost      & $aN\!\log N$\\
$C_{\mathrm{drop}}$   & Index drop cost          & $b$ (constant)\\
$c_{\mathrm{scan}}$   & Per-query cost w/o idx   & $fN\!=\!\Theta(N)$\\
$c_{\mathrm{idx}}$    & Per-query cost w/ idx    & $g\!\log N$\\
$Q$                   & Query volume next window & Sys.-dep.\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Structural Comparison with Prior Methods}\label{supp:features}

\begin{table*}[!t]\centering\small
\caption{Structural comparison with prior methods.
Metric: satisfies P1--P4. Multi-dim: orthogonal similarity dimensions.
Decision: cont.\ = continuous score + closed-form threshold.
Train-free: no labelled data or gradient-based training.}\label{tab:features-supp}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Method} & \textbf{Metric P1--P4} & \textbf{Multi-dim} & \textbf{Temporal} & \textbf{Decision} & \textbf{Train-free} & \textbf{Transition}\\
\midrule
...
\subsection{Workload Phase Mappings}\label{supp:phases}

Tables~\ref{tab:phases-supp} and~\ref{tab:mongo-phases-supp} list the
per-phase window assignments and dominant query/operation mixes for the
PostgreSQL TPC-H and MongoDB cross-engine workloads, respectively
(referenced from the main paper, Experimental Evaluation).

\begin{table}[!htb]\centering\small
\caption{TPC-H SF\,=\,0.2 workload phases (PostgreSQL~16).
$\Npts$ range reflects Poisson-sampled window sizes
(mean~19.5, range~16--20).}\label{tab:phases-supp}
...
($\Npts\!=\!20$), 10-block paired RCB.}\label{tab:mongo-phases-supp}
...
\subsection{Dimensional-Utility Table}\label{supp:dim-utility}

\begin{table}[!htb]\centering\scriptsize
\caption{Dimensional utility ($\Delta\!=\!\text{within}-\text{cross}$).
Every dimension achieves $\Delta\!>\!0.10$ in at least one workload,
validating Theorem~2 (Minimal Sufficient Dimensions) of the main paper.}\label{tab:dim-utility-supp}
...
\subsection{Dimension-Ablation Study}\label{supp:ablation}

\begin{table*}[!t]\centering\small
\caption{Discrimination ratio when each dimension is removed (weight
redistributed equally). TPC-H Poisson SF\,=\,1 (32 windows) and SDSS
(776 windows, 10 seeds), all-pairs protocol.}\label{tab:ablation-supp}
\setlength{\tabcolsep}{6pt}
...
$-\SA$  & 1.454 & $-7.2$  & 1.296 & $-1.1$  & Critical (TPC-H schema)\\
$-\ST$  & 1.394 & $-11.1$ & 1.122 & $-14.4$ & Critical (query-mix)\\
$-\SR$  & 1.465 & $-6.5$  & 1.346 & $+2.8$  & Critical (TPC-H rank)\\
$-\SV$  & 1.826 & $+16.5$ & 1.441 & $+10.0$ & Stable OLAP; critical in pgbench (A8c)\\
$-\SPER$& 1.720 & $+9.7$  & 1.380 & $+5.4$  & Steady; critical in burst (A8d)\\
\bottomrule
\end{tabular}
\end{table*}

\subsection{Research-Question Map}\label{supp:rq}

\begin{table}[!htb]\centering\scriptsize
\caption{Research questions and validating theorems.}\label{tab:rq-supp}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}llll@{}}
\toprule
...
\textbf{B3 (Cross-Engine $\theta$ Mismatch).}
To validate that $\theta$ must be per-engine calibrated, we measure precision when applying one engine's optimal $\theta$ to the other.
\begin{table}[h]\centering\footnotesize
\caption{Cross-engine $\theta$ mismatch (B3). Applying PostgreSQL-optimal $\theta\!=\!0.775$ to MongoDB or MongoDB-optimal $\theta\!=\!0.55$ to PostgreSQL degrades precision sharply, confirming Theorem~3's workload- and engine-specific prediction. Discussed in main paper (A-CE: Cross-Engine Validation on MongoDB).}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lllrl@{}}
\toprule
\textbf{Scen.} & \textbf{Engine} & $\theta$ & \textbf{Trig./Blk} & \textbf{Prec.}\\
\midrule
B3-A & PostgreSQL & 0.55 (Mongo-opt)    & 0           & N/A\\
B3-B & MongoDB    & 0.775 (PG-opt)      & $\approx$10 & 0.27\\
\bottomrule
\end{tabular}
\label{tab:cross-engine-theta}
\end{table}
...
\subsection{A13d Burst End-to-End: Extended Periodic Comparators ($K\!\in\!\{3,4\}$)}

The main paper's A13d burst end-to-end table (periodic $K\!=\!2$ as the
scheduled-baseline representative) is extended below.  For completeness, Table~\ref{tab:a13d-supp}
reports the same two operating points (Small-$Q$, Mid-$Q$) against
periodic $K\!\in\!\{3,4\}$, demonstrating that raising $K$ does not
close the gap to HSM-gated: although higher $K$ reduces advisor
calls (and therefore $t_{\mathrm{adv}}$), it keeps trigger precision
near zero on the burst workload because the rhythm of fixed-stride
polling does not align with the economic-optimum
transitions of Theorem~3 (main paper).  Block medians with 95\,\% bootstrap CIs
($B\!=\!10\,000$, paired within-block resamples, $n\!=\!10$ blocks).

\begin{table}[!htb]\centering\scriptsize
\caption{A13d supplementary: periodic $K\!\in\!\{3,4\}$ on the
burst workload, both operating points.  Columns identical to
the main paper's A13d burst end-to-end table.  Raw CSVs in
\texttt{results/burst\_end\_to\_end/}.}\label{tab:a13d-supp}
...
$P_{\mathrm{strict}}$ & $p_{95}^{\mathrm{lat}}$\,(ms) \\
\midrule
\multicolumn{7}{@{}l}{\emph{Small-$Q$: $Q\!=\!35$, $\theta\!=\!0.750$}}\\
 & Periodic $K\!=\!3$ (Dexter)              & $1.815$\,{\tiny[1.759,1.860]} & $0.774$\,{\tiny[0.74,0.81]} & $6$ & $0.000$ & $45.48$\,{\tiny[44.14,46.40]} \\
 & Periodic $K\!=\!3$ (\texttt{index\_adv.})& $1.395$\,{\tiny[1.331,1.447]} & $0.590$\,{\tiny[0.58,0.65]} & $6$ & $0.000$ & $39.18$\,{\tiny[32.93,39.96]} \\
 & Periodic $K\!=\!4$ (Dexter)              & $1.691$\,{\tiny[1.602,1.711]} & $0.656$\,{\tiny[0.60,0.70]} & $5$ & $0.000$ & $45.22$\,{\tiny[43.81,45.52]} \\
 & Periodic $K\!=\!4$ (\texttt{index\_adv.})& $1.163$\,{\tiny[1.132,1.305]} & $0.560$\,{\tiny[0.55,0.60]} & $5$ & $0.000$ & $31.93$\,{\tiny[30.73,35.24]} \\
\midrule
\multicolumn{7}{@{}l}{\emph{Mid-$Q$: $Q\!=\!700$, $\thetastar\!=\!0.826$}}\\
 & Periodic $K\!=\!3$ (Dexter)              & $35.88$\,{\tiny[35.63,36.32]} & $16.37$\,{\tiny[16.26,16.73]} & $139$ & $0.000$ & $43.19$\,{\tiny[42.93,43.49]} \\
 & Periodic $K\!=\!3$ (\texttt{index\_adv.})& $25.23$\,{\tiny[24.95,28.18]} & $12.67$\,{\tiny[12.50,12.89]} & $139$ & $0.000$ & $33.14$\,{\tiny[29.39,46.33]} \\
 & Periodic $K\!=\!4$ (Dexter)              & $32.00$\,{\tiny[31.89,32.48]} & $12.75$\,{\tiny[12.67,13.07]} & $104$ & $0.019$ & $42.88$\,{\tiny[42.79,43.03]} \\
 & Periodic $K\!=\!4$ (\texttt{index\_adv.})& $21.97$\,{\tiny[21.73,22.24]} & $9.71$\,{\tiny[9.68,9.81]} & $104$ & $0.019$ & $29.34$\,{\tiny[29.04,29.91]} \\
\bottomrule
\end{tabular}}
\end{table}

Observations: (i) at Small-$Q$, $K\!=\!3,4$ produce zero true-positive
triggers in all 10 blocks (the fixed-stride grid misses both synthetic
transitions); (ii) at Mid-$Q$, $K\!=\!3$ similarly misses both (hence
$P_{\mathrm{strict}}\!=\!0$) while $K\!=\!4$ catches at most one of
the 104 calls on a ground-truth boundary, giving
$P_{\mathrm{strict}}\!=\!0.019$---still two orders of magnitude worse
than HSM-gated's $1.000$ reported in the main paper (A13d);
(iii) raising $K$ monotonically reduces wall-clock but never beats
HSM-gated's $20.58$\,s (Dexter) / $14.65$\,s (\texttt{index\_adv.}),
confirming that the advantage of economic gating is not merely a
frequency reduction but a \emph{selective} frequency reduction aligned
with the $Q_i\!>\!Q_{\min}/(1\!-\!S_i)$
inequality of Theorem~3 (main paper).
...
$\pi_w$-parameterised speedup curve (SF\,=\,3.0 inputs); the full
curve and the 95\,\% deployment certificate at $\pi_w\!=\!0.95$ are
stated once in the Phase-Mix-Parameterised Speedup remark of
\S X (Theorem~5 Extended Remarks).

\begin{figure}[htbp]\centering
\includegraphics[width=\columnwidth]{figures/supp01_scale_analysis.pdf}
...
\begin{figure}[htbp]\centering
\includegraphics[width=\columnwidth]{figures/supp02_gate_sensitivity.pdf}
...
\begin{figure}[htbp]\centering
\includegraphics[width=\columnwidth]{figures/supp03_theta_calibration.pdf}
...
\subsection{Cross-Workload $\theta^{*}$ Generalisability}
\label{supp:theta-cal-xworkload}

To assess whether the operating point $\theta^{*}$ generalises beyond
the calibration workload, we extend the all-pairs within-vs.-cross
protocol to four heterogeneous workloads: TPC-H (PostgreSQL,
SF\,$=\!0.2$; $n\!=\!7{,}892$ pairs), SDSS ($n\!=\!4{,}954$), and the
two JOB variants---JOB-light ($n\!=\!210$) and JOB-complexity
($n\!=\!630$).  For every candidate $\theta\!\in\![0,1]$ on a $0.0025$
grid we compute TPR\,$=\!\Pr(\mathrm{score}\!<\!\theta\,\vert\,\mathrm{cross})$,
FPR\,$=\!\Pr(\mathrm{score}\!<\!\theta\,\vert\,\mathrm{within})$ and
Youden $J(\theta)\!=\!\mathrm{TPR}\!-\!\mathrm{FPR}$; the empirical
optimum is $\theta^{*}\!=\!\arg\max_{\theta} J(\theta)$.

Table~\ref{tab:theta-xworkload} reports per-workload
$(\theta^{*},\,J^{*},\,\mathrm{AUC})$ from
\texttt{results/theta\_optimal\_per\_workload.csv}; the corresponding
ROC and $J(\theta)$ overlays appear in
Figure~\ref{fig:theta-cal-xworkload}.

\begin{table}[htbp]\centering\small
\caption{Cross-workload Youden-optimal $\theta^{*}$, $J^{*}$, and AUC
(all-pairs within-vs.-cross protocol).  Source:
\texttt{results/theta\_optimal\_per\_workload.csv}.}
\label{tab:theta-xworkload}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Workload} & \boldmath$n_{\mathrm{pairs}}$ & \boldmath$\theta^{*}$ & \boldmath$J^{*}$ & \textbf{AUC}\\
\midrule
TPC-H (SF$=0.2$)  & 7{,}892 & 0.605  & 0.498 & 0.788\\
SDSS              & 4{,}954 & 0.6325 & 0.477 & 0.810\\
JOB-light         &     210 & 0.7275 & 0.331 & 0.705\\
JOB-complexity    &     630 & 0.775  & 0.464 & 0.789\\
\midrule
Min--Max          &  ---    & 0.605--0.775 & 0.331--0.498 & 0.705--0.810\\
\bottomrule
\end{tabular}
\end{table}

\begin{figure}[htbp]\centering
\includegraphics[width=\columnwidth]{figures/supp03b_roc_cross_workload.pdf}
\caption{Cross-workload $\theta$ calibration (all-pairs protocol).
\textbf{(A)} Per-workload ROC curves; star marks the Youden-optimal
operating point $\theta^{*}$.  \textbf{(B)} Youden $J(\theta)\!=\!$
TPR$-$FPR.  All four workloads place $\theta^{*}$ in the band
$[0.61,0.78]$, bracketing the default operating point
$\theta_{0}\!=\!0.75$ and the PostgreSQL engine-calibrated
$\theta_{\mathrm{Pg}}^{*}\!=\!0.775$ from
Fig.~\ref{fig:theta-cal-supp}.}
\label{fig:theta-cal-xworkload}
\end{figure}

\paragraph{Findings.}
$(i)$ The four $\theta^{*}$ values span $0.605$--$0.775$
(range\,$=\!0.17$, CV\,$=\!11.0\%$), supporting the choice of
$\theta_{0}\!=\!0.75$ as a reasonable default that lies within the
empirical band for every workload tested.
$(ii)$ AUC remains in the band $[0.71, 0.81]$ across workloads, with
the largest separation on SDSS (AUC\,$=\!0.810$) and the tightest on
JOB-light (AUC\,$=\!0.705$, $n\!=\!210$ -- the smallest sample);
the discrimination ratio rank thus tracks pair-count rather than
workload type, consistent with finite-sample variance.
$(iii)$ The all-pairs protocol used here is intentionally more
adversarial than the per-window detection protocol that produced the
TPC-H $J\!\ge\!0.96$ figures in main-paper \S H Table~12 (A14 row):
all-pairs evaluates discrimination across every within/cross
combination, whereas per-window detection collapses each window into a
single trigger decision against ground-truth phase boundaries.  Both
protocols yield $\theta^{*}$ in the $[0.6, 0.8]$ band, supporting
Theorem~3 (Economic Optimality)'s parameter-invariance prediction
(operating-point invariance up to data-model normalisation).

% Sanity / provenance:
%   - within_cross_phase.csv contains a section tagged ``tpch'' whose
%     scores are byte-identical to job_hsm_execute_pair_scores.csv (a
%     known data-pipeline mislabelling).  We therefore use
%     score_distribution.csv as the canonical TPC-H pair-scores source.

\textbf{B4 (Amplitude Invariance).} To confirm that the angular kernel is truly amplitude-invariant, we scale the receipt workload by factors $k\in\{0.5,1,2,5,10\}$ and measure the resulting HSM score dimensions. The rank ($S_R$) and temporal ($S_T$) dimensions should remain constant, while the volume dimension ($S_V$) varies proportionally to $k$.

\begin{figure}[htbp]\centering
\includegraphics[width=\columnwidth]{figures/supp06_amplitude_invariance.pdf}
\caption{In-phase amplitude invariance. $S_R$ and $S_T$ remain constant across amplitude scaling $k \in \{0.5, 1, 2, 5, 10\}$ (10 blocks each), confirming angular kernel insensitivity to workload volume changes within a phase. $S_V$ varies with $k$ as expected. HSM floor ranges from 0.695 to 0.875.}
\label{fig:supp-amplitude}
\end{figure}

\textbf{B5 (Weight and Noise Robustness).} To assess HSM's robustness to simultaneous weight and noise perturbations, we construct a 2D grid varying noise level $z$ and weight perturbation magnitude $\delta$ independently. We track both the optimal threshold $\theta^*$ (which should remain stable) and the AUC (which may degrade at large perturbations).

\begin{figure*}[htbp]\centering
\includegraphics[width=\textwidth]{figures/supp07_noise_weight_grid.pdf}
\caption{Robustness grid: (a)~$\theta^*$ and (b)~AUC across noise level $z$ and weight perturbation $\delta$. $\theta^*$ remains stable ($0.882$--$0.920$, CV${<}1\%$) across all perturbations. AUC degrades at $\delta \geq 0.075$.}
\label{fig:supp-noise-weight}
\end{figure*}

\FloatBarrier

%%─────────────────────────────────────────────────────────────
\section{A13 Decision-Layer Cost (Full Tables and Figures)}
\label{supp:a13}
%%─────────────────────────────────────────────────────────────

This section reports the full per-SF wall-QPS table and the
throughput-comparison figure summarised in main paper~\S A13, plus
the cross-workload speedup-bound table (A13c) and the kernel-ablation
figure for the burst-mid-$Q$ workload.
...
($K\!=\!3$). Both ratios are scale-invariant (CV\,$<$\,7\,\%) across the $15\times$
data-volume range.}\label{tab:throughput-supp}
...
\includegraphics[width=\columnwidth]{../main/figures/fig07_throughput_comparison.pdf}
\caption{Decision-layer cost across invocation policies (10-block RCB,
$n\!=\!10$ per cell). All conditions execute on identical schemas; bars
isolate the in-band cost of each policy (decision layer $+$ triggered
advisor work) as a within-block paired ratio versus the no-advisor
baseline (95\,\% percentile bootstrap CI).
...
\label{fig:throughput-supp}
\end{figure}

\begin{table}[!htb]\centering\scriptsize
\caption{A13c: Cross-workload empirical validation of
Theorem~5 (operational speedup with phase-mix prior).
Each row is one workload at the $\theta$ used for the deployment run.
$N$\,=\,number of consecutive window-pairs;
$\hat{p}_{\mathrm{stable}}$\,=\,\emph{empirical} fraction of retained
window-pairs from the Hoeffding-certified estimator (not the oracle
phase-mix);
Bnd\,=\,$1/(1\!-\!\hat{p}_{\mathrm{stable}})$
(asymptotic upper bound of Theorem~5 evaluated at the empirical
estimate; the certificate is valid w.h.p.\ via the $\eta$ band);
TPR\,=\,gate sensitivity against phase labels;
sav\textsubscript{A}/sav\textsubscript{P}\,=\,realised invocation
savings versus always-on / periodic ($K\!=\!3$). TPC-H entries aggregate
$10$-block RCB across SF\,$\in\!\{0.2,1.0,3.0\}$.
All rows are regenerated from
\texttt{results/t5\_validation.csv} in the HSM\_gated v5
artifact.}\label{tab:t5-cross-supp}
\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{@{}lrrrrrrr@{}}
\toprule
\textbf{Workload} & $\theta$ & $N$ &
$\hat{p}_{\mathrm{stable}}$ & Bnd & TPR &
sav\textsubscript{A} & sav\textsubscript{P}\\
\midrule
TPC-H (3 SFs)    & 0.750 & 240  & 0.808 & $5.22\times$ & ---  & $80.8\,\%$ & $42.5\,\%$\\
Burst            & 0.750 & 26   & 0.808 & $5.20\times$ & 0.50 & $80.8\,\%$ & $44.4\,\%$\\
SDSS ($\thetastar$) & 0.6325 & 4954 & 0.751 & $4.01\times$ & 0.60 & $75.1\,\%$ & $25.2\,\%$\\
JOB ($\thetastar$, small $n$) & 0.7275 & 20  & 0.950 & $20.0\times$ & 0.00 & $95.0\,\%$ & $85.7\,\%$\\
OLTP (pgbench)   & 0.750 & 37   & 0.000 & $1.00\times$ & 1.00 & $0.0\,\%$  & ---\\
\bottomrule
\end{tabular}
\end{table}

\begin{figure*}[!t]\centering
\includegraphics[width=0.70\textwidth]{../main/figures/fig08_kernel_ablation.pdf}
\caption{Kernel ablation on the burst-mid-$Q$ workload
($n_w\!=\!4170$, $n_c\!=\!20$, 10 blocks). Angular and cosine kernels
achieve identical $J^{*}\!=\!0.667$ and AUC\,$=\!0.833$ at
$\theta^{*}\!\approx\!0.9$. The Euclidean distance attains
$J^{*}\!=\!1.0$ (AUC\,$=\!1.0$), an outcome consistent with
overfitting to the limited cross-phase sample. Fig.~\ref{fig:supp-amplitude}
confirms amplitude invariance across workload scaling, and
Fig.~\ref{fig:supp-noise-weight} validates robustness to weight
perturbations.}
\label{fig:kernel-ablation-supp}
\end{figure*}
...
$\eta_{m,\delta}$ & Stratified-Hoeffding half-width $\sqrt{\log(4/\delta)/(2m)}$\\
