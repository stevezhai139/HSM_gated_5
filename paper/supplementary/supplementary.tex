...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Scenario} & \textbf{Engine} & $\theta$ & \textbf{Triggers/Block} & \textbf{Precision}\\
\midrule
B3-A & PostgreSQL & 0.55\,(Mongo-opt) & 0 & N/A\,(no fires)\\
...
Main Table~\ref{tab:a13d-results} uses periodic $K\!=\!2$ as the
scheduled-baseline representative.  For completeness, Table~\ref{tab:a13d-supp}
...
polling does not align with the Theorem~\ref{thm:econ} economic
transitions.  Block medians with 95\,\% bootstrap CIs
...
main Table~\ref{tab:a13d-results}.  Raw CSVs in
...
than HSM-gated's $1.000$ in main Table~\ref{tab:a13d-results};
...
with Theorem~\ref{thm:econ}'s $Q_i\!>\!Q_{\min}/(1\!-\!S_i)$
inequality.
...
SF\,$=\!0.2$; \texttt{score\_distribution.csv}, $n\!=\!7{,}892$
pairs), SDSS (\texttt{sdss\_hsm\_pair\_scores.csv}, $n\!=\!4{,}954$),
and the two JOB variants---JOB-light
(\texttt{job\_hsm\_execute\_pair\_scores.csv}, $n\!=\!210$) and
JOB-complexity (\texttt{job\_hsm\_complexity\_execute\_pair\_scores.csv},
