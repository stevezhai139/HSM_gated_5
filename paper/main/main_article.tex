% IEEE Transactions on Knowledge and Data Engineering (TKDE) — IEEEtran journal template
% Per TKDE admin-check email 2026-04-20: manuscript must be reformatted using IEEEtran.cls
...
\usepackage{soul}
\title{HSM: Workload-Similarity Gating for Index-Maintenance Decisions, with Formal Bounds}

\author{Arun~Reungsilpkolkarn%
\thanks{A.~Reungsilpkolkarn is with the School of Information Technology and Innovation, Bangkok University, Pathum Thani, Thailand. E-mail: arun.r@bu.ac.th. ORCID: 0009-0009-2077-0006.}%
\thanks{Manuscript submitted to IEEE Transactions on Knowledge and Data Engineering, 2026.}}

\markboth{IEEE Transactions on Knowledge and Data Engineering, Vol.~XX, No.~X, 2026}%
{Reungsilpkolkarn: HSM: Workload-Similarity Gating for Index-Maintenance Decisions}

\begin{document}
\maketitle

\begin{abstract}
Existing index advisors (Dexter, Supabase
...
certificate, and a convex-optimal window size.
On PostgreSQL\,16 with TPC-H (SF\,0.2--3.0), HSM attains
Youden $J\!\ge\!0.96$ at every scale on the phase-labelled
retain-vs-rebuild detection protocol (within-phase vs.\ cross-phase
window pairs, $m_w\!=\!m_c\!=\!160$, \S V.A14), and reduces advisor
invocations by $80.8\%$ versus always-on and $42.5\%$ versus periodic
...
\begin{IEEEkeywords}
Decision layer, index-maintenance timing, proactive index management, workload similarity, hierarchical similarity measurement, discrete wavelet transform, self-tuning databases.
\end{IEEEkeywords}
...
window under which the index was last calibrated.
...
bounds, a deployment speedup certificate, and a convex-optimal
...
(Theorem~\ref{thm:econ}, part~(ii); supplementary~\S VII).
\textbf{C4}~A closed-form threshold $\thetastar(N,Q)$ validated
across SF\,0.2--3 (Youden $J\!\ge\!0.96$ at every scale under the
phase-labelled detection protocol) and generalising across TPC-H,
burst, and cross-engine workloads.
...
\emph{Gap~2.1: no prior method jointly provides a continuous
multi-dimensional similarity score with formal metric properties, a
closed-form decision threshold, and a training-free deployment model.}
...
\textit{Change-point detection (CPD).} Classical sequential CPD
includes CUSUM~\cite{Page1954CUSUM} and Bayesian Online Change-Point
Detection~\cite{AdamsMacKay2007BOCPD}, which optimise a
detection-delay/false-alarm trade-off by scanning a \emph{stream}
position-by-position. HSM differs in protocol: it is a
\emph{window-pair similarity classifier} that decides retain-vs-rebuild
on aligned windows, not on stream positions (Theorem~4 reformulates
the gate as a binary detector under this protocol). The two families
are complementary---streaming CPD can supply windows to HSM
dynamically, while HSM supplies a DBMS-agnostic composite similarity
that single-signal CPD does not provide---and are therefore not
substitutable baselines.
...
\emph{Gap~2.2: no prior work provides formal guarantees for multi-scale
workload similarity---neither metric properties nor parameter optimality bounds.}
...
\emph{Gap~2.3: no existing framework (B-Trees, cracking, learned indexes,
WaZI) formally characterises access-attribute independence or provides
multi-dimensional temporal similarity with cost guarantees.}
...
\emph{Gap~2.4: no existing framework provides a DBMS-agnostic workload
similarity measure validated across SQL and document-store engines with
...
closes the framework with the convex-optimal window size $\Nstar$
...
$P(\mathrm{misclass.})<1/\alpha\!=\!1/8$.\footnote{The deployed pipeline
uses $\alpha\!=\!k\!=\!4$: DWT detail bands $cD_1$--$cD_3$ encode burst
direction, leaving SAX on $cA_3$ to discriminate only the $k$ canonical
states; $\alpha^{*}\!=\!2k$ is the SAX-only fallback (Supp.~\S II).}
\end{lemma}
...
\noindent\textit{Empirical caveat (global sufficiency vs.\ per-workload utility).}
Theorem~\ref{thm:minimal} establishes \emph{global} sufficiency and
minimality: the witness construction guarantees that no four-dimensional
subset suffices on the joint workload space. This does \emph{not}
imply every dimension is equally informative on every single workload.
On TPC-H, $\Delta\SPER\!\approx\!0$ across all SFs
(vs.\ $\Delta\ST\!\approx\!0.53$, $\Delta\SA\!\approx\!0.56$), so
$(\ST,\SA)$ dominate phase separation; we retain $\SPER$ because
$(i)$~it carries unique intra-window shape information required by
the burst witness pair (necessary for global sufficiency in
Theorem~\ref{thm:minimal}), and $(ii)$~workloads with temporal
periodicity make $\SPER$ informative (14\,\% contribution on burst
A8d). Per-dimension utility across five workloads is quantified in
the Supplementary (Dimensional-Utility Table).

\subsection{Theorem 3: Economic Optimality of HSM Gating}

The cost model uses system-measurable constants $a,b,f,g,c$
(full parameter table in the Supplementary, Cost-Model Parameters).
...
A structural comparison against B-tree, cracking, learned indices, and
recent workload-similarity methods (BALANCE, Indexer++, ShapeShifter,
Brucato) appears in the Supplementary (Cost-Model Parameters;
Structural Comparison): HSM is the only method that simultaneously
satisfies P1--P4, exposes five orthogonal dimensions, supplies a
continuous score with a closed-form threshold, requires no labelled
training data, and amortises advisor cost to $O(1)$ per window.
...
operating point.}
The imperfect empirical rates ($\hat\TPR\!=\!0.986$,
$\hat\TNR\!=\!0.970$) are a direct consequence of the metric
construction (Lemma~\ref{lem:metric}): the angular transforms
$d_R$, $d_T$ satisfy the triangle inequality \emph{precisely because}
they are locally linear near identity, so small distributional
deviations map to proportional metric distances rather than being
compressed into a plateau. HSM therefore reacts to genuine
within-phase sampling variance, the desirable behaviour of a
metric-compliant detector. Consequently $\theta$ is an
\emph{operating point} on the detector's ROC curve rather than a
universal constant: Supplementary~\S XIV reports that the Youden-optimal
$\thetastar$ varies across engines and workloads (TPC-H, JOB/IMDB, SDSS,
OLTP), so operators can raise $\theta$ on highly regular workloads or
lower it for early warning on subtle drift. The default
$\theta\!=\!0.75$ is the Youden-optimum on aggregated TPC-H+JOB
calibration and dominates periodic rebuild at every evaluated SF.
...
Numerical values at SF\,=\,3.0 (worst case; $\hat\TPR\!=\!0.986$,
$\hat\TNR\!=\!0.970$) are reported once in \S V.A14
(Theorem-5 certificate and the $\pi_w$-parameterised curve);
Supplementary~\S XII gives the full per-SF tables and the scale
analysis figure.

\smallskip
\noindent\textit{Applicability to state-of-the-art advisors.}
The premise $T_{\mathcal{A}}(N)\!=\!\Omega(N\log N)$ is satisfied by
\emph{every} published advisor we surveyed
(Table~\ref{tab:advisor-applicability}): each touches $\Omega(N)$ rows
to evaluate, build, or amortise a candidate index.
Theorem~\ref{thm:speedup} therefore applies \emph{orthogonally}---HSM
inserts a decision (\emph{when}) layer in front of any
index-selection (\emph{what}) advisor without modifying its internals.
End-to-end validation uses Dexter~\cite{AnkaneDexter} and
\texttt{index\_advisor}~\cite{SupabaseIndexAdvisor}; the same protocol
extends to GPU-trained advisors (BALANCE, MFIX, Indexer++).
...
\end{equation}
where (i)~$a\Npts\log \Npts$ is a conservative upper bound on the
measurement cost that accommodates kernel variants beyond the default
FastDTW-based pipeline of Lemma~\ref{lem:complexity} (which establishes
the tight $\Theta(\Npts)$ bound, a strict special case of
$a\Npts\log \Npts$); using this conservative form makes Theorem~6
robust to slower DWT/DTW variants, and $a\!>\!0$ is the per-point
HSM constant; (ii)~the
...
depth, and $C_{\mathrm{db4}}\!=\!\sqrt{2}/2\!\approx\!0.707$ the
Daubechies-4 attenuation constant of Lemma~\ref{lem:wcb}.
Then $\mathcal{C}$ admits a unique global minimum
$\Nstar$ characterised by the stationarity condition
$\mathrm{d}\mathcal{C}/\mathrm{d}\Npts\!=\!0$, which, after rearrangement,
yields the implicit fixed-point equation
\begin{equation}\label{eq:optwin-Nstar}
\resizebox{0.82\columnwidth}{!}{$\displaystyle
\Nstar\,=\,\sqrt{\frac{8\,C_{\mathrm{db4}}^{\,2L}\,M^{2}\,a\,\Nstar\log\Nstar}{\lambda(f_{\mathrm{N}}\!-\!g\log\Nstar)}}\;\;\text{(solve numerically)}$}
\end{equation}
in a single scalar $\Nstar$. $\mathcal{C}$ is strictly convex in $\Npts$
for all operating regimes with $g\log \Npts\!<\!f_{\mathrm{N}}$
(i.e.\ the linearised TPR approximation is valid), so $\Nstar$ is the
unique root of $\mathrm{d}\mathcal{C}/\mathrm{d}\Npts\!=\!0$ in
$[N_{\min}^{\mathrm{pts}},N_{\max}^{\mathrm{pts}}]$, where
$N_{\min}^{\mathrm{pts}}\!\geq\!8$ ensures sufficient sample size for
rank-based statistics ($\SR$, Spearman~$\rho$) and SAX
discretisation.\footnote{The real-valued solution is rounded to the
nearest admissible integer
$\lceil\Nstar\rceil$ or $\lfloor\Nstar\rfloor$, whichever yields
lower $\mathcal{C}$; convexity guarantees the rounded value is within
one unit of optimal.} Bisection or fixed-point iteration on
Eq.~\eqref{eq:optwin-Nstar} converges in $O(\log(1/\epsilon))$ steps
...
$\lambda(N)\!=\!\Theta(N\log N)$ (the premise
$T_{\mathcal{A}}(N)\!=\!\Omega(N\log N)$ of Theorem~\ref{thm:speedup}),
and assume $M$, $a$,
$f_{\mathrm{N}}$, $g$, $L$ are fixed design constants. Then the
convex-optimal $\Nstar$ of Theorem~\ref{thm:optwin} is invariant in $N$
...
match the Theorem~\ref{thm:optwin} prediction within one dyadic step, and the
...
retains $43.4\!\pm\!2.0\,\%$. Equivalently, HSM fires only
$4.6\!\pm\!1.4$ times per 24-window block (vs.\ $24$ and $8$ for
always-on and periodic); Table~\ref{tab:summary} rows A13/A13c
report the corresponding invocation-savings percentages. The decision-layer ratios are
...
Theorem~\ref{thm:speedup} bound of $5.22\times$; the realised
...
(OLTP, burst, JOB/IMDB, SDSS) at each workload's empirical
$\hat{p}_{\mathrm{stable}}$ (CSV of record:
\texttt{results/t5\_validation.csv}): burst at $\theta\!=\!0.75$ attains
$80.8\,\%/44.4\,\%$ savings under a $5.20\times$ bound; SDSS at
$\thetastar\!=\!0.6325$ (Youden) attains $75.1\,\%/25.2\,\%$ under a
$4.01\times$ bound; JOB/IMDB at $\thetastar\!=\!0.7275$
($n\!=\!20$, sample-limited) attains $95.0\,\%/85.7\,\%$ under a
$20.0\times$ bound. OLTP correctly self-deactivates: pgbench drifts
on essentially every window, so
$\hat{p}_{\mathrm{stable}}\!\to\!0$ and the bound itself collapses to
...
A12 & T5   & $\beta\!=\!1.177$ ($R^2\!=\!0.998$); $\Theta(N\log N)$ rebuild scaling ($N_{\mathrm{lineitem}}\!\in\![1.2\,\mathrm{M},18\,\mathrm{M}]$, SF\,0.2--3.0). Kernel-cost micro-benchmark over $\Npts\!\in\![100,30{,}000]$ gives $\beta\!=\!1.094$ ($R^2\!=\!0.9999$). & $\checkmark$\\
...
A13c & T5 & Bound $1/(1\!-\!\hat{p}_{\mathrm{stable}})$ realised on TPC-H ($5.22\times$, $80.8\%/42.5\%$), burst ($5.20\times$, $80.8\%/44.4\%$), SDSS ($4.01\times$, $75.1\%/25.2\%$), JOB ($20.0\times$, $95.0\%/85.7\%$); OLTP correctly diagnosed as high-drift ($\hat{p}_{\mathrm{stable}}\!\to\!0$, bound self-deactivates) & $\checkmark$\\
...
A17 & T6   & Optimal $\Npts$ from convex cost model; empirical min at $\Npts\!=\!50$ (TPC-H SF\,0.2, SF\,3.0); $\Npts\!=\!100$ (JOB) & $\checkmark$\\
\midrule
...
(Theorems~\ref{thm:gate},~\ref{thm:speedup}).}
OLS over $N_{\mathrm{lineitem}}\!\in\![1.2,18]\mathrm{M}$ (SF\,0.2--3.0)
yields $\beta\!=\!1.177$ ($R^2\!=\!0.998$) for rebuild cost $T_A(N)$
and $\beta\!=\!1.094$ ($R^2\!=\!0.9999$) for kernel cost
$T_{\mathrm{HSM}}(\Npts)$ over $\Npts\!\in\![100,30{,}000]$; both are
consistent with the Lemma~\ref{lem:complexity} bounds (the finite-range
super-linear residual absorbs FastDTW path length and memory-hierarchy
effects without a genuine $\log\Npts$ factor), and the gap between
them sources the $O(\Npts/(N\log N))$ amortisation of Theorem~5.
Youden $J\!\ge\!0.9626$ at every scale ($J^{*}\!=\!0.9684$ at
SF\,=\,0.2, $0.9626$ at SF\,=\,3.0, both at $\thetastar\!=\!0.775$).
Theorem~\ref{thm:speedup} gives a 95\,\% certificate
$[5.72\times,23.45\times]$ at $\pi_w\!=\!0.95$ (SF\,=\,3.0 inputs),
central $16.1\times$, rising to $41.9\times$ at $\pi_w\!=\!0.99$.
Full per-SF tables, Hoeffding bands, and ROC sweeps: Supplementary~\S XII.
...
convex-optimal window size.  Experiments confirm each theorem
(Table~\ref{tab:summary}): scale-invariant DR across SF\,0.2--3.0,
$<\!0.3\%$ overhead, and a Theorem-5 95\% deployment certificate
$[5.72\times, 23.45\times]$ at $\pi_w\!=\!0.95$ on the worst-case
SF\,=\,3.0 inputs.
...
(ii)~The fixed weights $\mathbf{w}_0$ deliberately target
cross-workload consistency rather than peak per-workload Youden~$J$:
on workloads where a single dimension carries the phase signal
(e.g.\ $\ST$ on SDSS), a $\mathbf{w}_0$-tuned single-dimension
detector matches HSM on peak $J$ (Table~\ref{tab:baselines}) but
forfeits the composite metric properties of
Lemma~\ref{lem:metric} (triangle inequality required by the
Theorem~\ref{thm:econ} break-even analysis) and the
dimension-wise diagnosis of Algorithm~2. HSM therefore retains the
five-dimensional composite as the default; automated per-workload
weight calibration that preserves P1--P4 is left to future work.
(iii)~End-to-end throughput is reported only for
SF\,$\in\{0.2,1.0,3.0\}$ (up to $\approx\!18$\,M rows);
the kernel-measurement cost is tight-$\Theta(\Npts)$
(Lemma~\ref{lem:complexity}) and extrapolates, but end-to-end runs at
SF\,$\geq\!10$ are dominated by buffer-pool, autovacuum, and
unindexed-join effects outside the kernel and are deferred to future work.
...
A complete replication package~\cite{HSM2026Repo}---source code,
configuration, scripts, and instructions to reproduce all reported
results---is publicly available at
\url{https://github.com/stevezhai139/HSM_gated_5} (BSD-3-Clause).
The package includes a five-minute reviewer quickstart (CPU-only path,
no database required), the metric-axiom unit tests for Lemma~1, the
HSM kernel and $\thetastar$ implementation, per-workload runners
(TPC-H, SDSS, JOB/IMDB, pgbench, burst), the end-to-end Dexter and
Supabase \texttt{index\_advisor} harnesses, and the shipped CSVs that
back every figure and table in the paper. Step-by-step reproduction
of the PostgreSQL\,16 and MongoDB\,7 end-to-end runs is documented in
\texttt{REPRODUCE.md}. A versioned snapshot will be archived at Zenodo
upon acceptance.
